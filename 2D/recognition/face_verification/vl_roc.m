function [tpr,tnr,info] = vl_roc(scores, labels, varargin)
%VL_ROC   ROC curve.
...
if nargout > 2 || nargout == 0
 ...
 % calc tpr @ fpr==0.01(tnr==0.99)
  ...
  neg_num = numel(find(labels == 0));
  % calc tpr @ fpr==0.001(tnr==0.999)
  if(neg_num >= 1000)
      s = max(find(tnr >= 0.999)) ;
      if s == length(tpr)
          info.tpr0001 = NaN ;
      else
          info.tpr0001 = tpr(s);
  ...
  % calc tpr @ fpr==0.0001(tnr==0.9999)
  if(neg_num >= 10000)
      s = max(find(tnr >= 0.9999)) ;
      if s == length(tpr)
          info.tpr00001 = NaN ;
      else
          info.tpr00001 = tpr(s);
  ...
  % calc tpr @ fpr==0.00001(tnr==0.99999)
  if(neg_num >= 100000)
      s = max(find(tnr >= 0.99999)) ;
      if s == length(tpr)
          info.tpr000001 = NaN ;
      else
          info.tpr000001 = tpr(s);
      end
  end 

  % calc tpr @ fpr==0.000001(tnr==0.999999)
  if(neg_num >= 1000000)
      s = max(find(tnr >= 0.999999)) ;
      if s == length(tpr)
          info.tpr0000001 = NaN ;
      else
          info.tpr0000001 = tpr(s);
      end
  end 
