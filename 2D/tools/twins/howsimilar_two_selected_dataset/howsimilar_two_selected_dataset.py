...
class Post(object):
    ...
    def how_similar(self, leftID, rightID):
        ...
        req = urllib2.Request(http_url)
        data = urllib.urlencode(form_data)
        resp = urllib2.urlopen(req, data, timeout=100)
        print("Parsing score...")
        return self.parse_score(resp.read())
def get_faceID(objPost, path):
    ...
    while cn<3:                
        try:
            ...
            print 'error in get_faceID function\n'
            cn = cn+1
            print
    return faceID, cn
...
def compute_folder_similarity(first_dir, second_dir,output_dir, first_path, second_path):
...
def FindFacePair(first_dir, second_dir,output_dir, thread_num, thread_ID, dir_txt):
    with open(dir_txt,'rt') as f:
        all_lines = f.readlines();
    thread_per_ID = len(all_lines)/thread_num
    start_index = thread_per_ID*thread_ID
    end_index = thread_per_ID*(thread_ID+1)
    for i_f in range(start_index,end_index):
            line = all_lines[i_f]
            line = line.strip('\n')
            print('the number of lines in dir_txt:%d\n' %(i_f))
            compute_folder_similarity(first_dir, second_dir, output_dir, line.split(' ')[0], line.split(' ')[1])
        

# 主函数测试代码
if __name__ == '__main__':
    first_dir = '/home/scw4750/Dataset/selected_asia_and_webface/asia'
    second_dir = '/home/scw4750/Dataset/selected_asia_and_webface/webface'
    output_dir = '/home/scw4750/Dataset/selected_asia_and_webface/similarity_txt'
    dir_txt = '/home/scw4750/Dataset/selected_asia_and_webface/dir_for_twins.txt'
    def sslwrap(func):
        @wraps(func)
        def bar(*args, **kw):
    ...
    for i in range(num_thread):
        t = threading.Thread(target = FindFacePair, args=(first_dir, second_dir, output_dir,num_thread,i, dir_txt))
        t.start()
