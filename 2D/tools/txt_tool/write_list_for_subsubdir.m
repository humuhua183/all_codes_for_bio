...
folder_struct = dir(img_dir);
folder_struct = folder_struct(3:end);

for i_f = 1:length(folder_struct)
    folder_name = folder_struct(i_f).name;

img_struct=dir([folder_struct(i_f).folder filesep folder_name]);
...
for i=1:length(img_struct)
    ...
    if isfield(param,'begin_number') 
       label = i - 1 + param.begin_number - empty_folder;
    else
       label = i - 1;
    end

    end
    sub_img_struct=dir([img_dir filesep folder_name filesep img_struct(i).name filesep filter]);
    if strcmp(filter,'')
        sub_img_struct = sub_img_struct(3:end);
    end
    if length(sub_img_struct) == 0
        empty_folder = empty_folder + 1;
    end
    for i_s=1:length(sub_img_struct)
        ...
        output_name=[folder_name filesep img_struct(i).name filesep output_name];
